...
static ssize_t keyboardleds_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	u8 state[128];
	int i;
	...
	int ret = hp_wmi_perform_query(0x02, HPWMI_KBDLED, &state,
				       sizeof(state), sizeof(state));
	if (ret) {
		pr_info("got ret 0x%x\n", ret);
		return 1;
	}

	return sprintf(buf, "#%02x%02x%02x #%02x%02x%02x #%02x%02x%02x #%02x%02x%02x\n", state[25], state[26], state[27], state[28], state[29], state[30], state[31], state[32], state[33], state[34], state[35], state[36]);
}

static ssize_t keyboardleds_store(struct device *dev, struct device_attribute *attr,
			 const char *buf, size_t count)
{
	...
	j = 0;
	for (i = 0; i < 4; i++) {
		ret = sscanf(buf + j, "#%2x%2x%2x ", &red, &green, &blue);
		if (!ret)
			return -EINVAL;
		state[pos++] = (u8) red;
		state[pos++] = (u8) green;
	...
	ret = hp_wmi_perform_query(0x03, HPWMI_KBDLED, &state,
				       sizeof(state), sizeof(state));
	if (ret) {
		pr_info("got ret 0x%x\n", ret);
		return -EINVAL;
